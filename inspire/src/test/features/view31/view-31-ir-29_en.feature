Feature: Requirement 29
	As the Metadata Date is not supported by **ISO 19128** – WMS 1.3.0, an extension shall be used to map this to 
	an <inspire_common:MetadataDate> element within an <inspire_vs:ExtendedCapabilities> element. The date shall 
	be expressed in conformity with the **INS MD**.
    !!!
	...
	Scenario: check if there has been correctly used inspire_common:MetadataDate element in case it was needed 
	...
	Then there is a inspire_common:MetadataDate node in the inspire_vs:ExtendedCapabilities section
